...
<!--Проект Kanban Board можно запустить двумя способами:
Через Docker Compose (рекомендуемый способ):
docker-compose up -d
Через npm (из корня проекта):
npm run start
После запуска:
- 
...
Требуются установленные Docker и Docker Compose. Для разработки также понадобится Node.js 20+.-->
